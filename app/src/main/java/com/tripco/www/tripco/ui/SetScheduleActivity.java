    ...
    public void onClickBtns(View view){
        ...
        switch (view.getId()){
            case R.id.toolbar_right_btn:
                if(rbsGroup.getCheckedRadioButtonId() == R.id.category1) cateNo = 1;
                else if(rbsGroup.getCheckedRadioButtonId() == R.id.category2) cateNo = 2;
                else cateNo = 0; // 체크를 안했거나 관광에 체크했으면 0
                if(!TextUtils.isEmpty(titleEt.getText())) title = titleEt.getText().toString();
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == U.PLACE_AUTOCOMPLETE_REQUEST_CODE) {
            if (resultCode == RESULT_OK) {
                ...
                CameraPosition ani = new CameraPosition.Builder().target(latlng).zoom(16).build();
